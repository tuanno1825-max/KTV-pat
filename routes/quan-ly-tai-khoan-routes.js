...
async function damBaoTaiKhoanCuCoMa() {
  if (!tienTrinhCapMaTaiKhoanCu) {
    tienTrinhCapMaTaiKhoanCu = NguoiDung.capMaKhachHangChoTaiKhoanCu().catch(
      (error) => {
        tienTrinhCapMaTaiKhoanCu = null;
        throw error;
      },
    );
  }
  await tienTrinhCapMaTaiKhoanCu;
}
...
router.get("/admin/tai-khoan", yeuCauAdmin, async (req, res) => {
  try {
    ...
    const grouped = await DonHang.aggregate([
      {
        $group: {
          _id: { email: "$emailKhach", trangThai: "$trangThai" },
          soLuong: { $sum: 1 },
        },
      },
    ]);
    ...
    for (const row of grouped) {
      const email = row._id.email?.toLowerCase();
      if (!email) continue;
      ...
      if (row._id.trangThai === "Đặt phòng thành công")
        s.thanhCong += row.soLuong;
      else if (row._id.trangThai === "Khách không đến")
        s.khongDen += row.soLuong;
      else if (row._id.trangThai === "Khách đã hủy đặt phòng")
        s.daHuy += row.soLuong;
      else if (["Đang chờ xử lý", "Đã xác nhận"].includes(row._id.trangThai))
        s.choXuLy += row.soLuong;
      else if (row._id.trangThai === "Đặt phòng thất bại")
        s.thatBai += row.soLuong;
      stats.set(email, s);
    }
    res.json(
      users.map((u) => ({
        ...
        laVip:
          u.vipTrangThai === "VIP" &&
          (!u.vipHetHan || new Date(u.vipHetHan) > new Date()),
        canhBao: u.canhBao || "",
        thongKeDonHang: stats.get(u.email.toLowerCase()) || {
          tongDon: 0,
          thanhCong: 0,
          khongDen: 0,
          daHuy: 0,
...
router.get("/admin/tai-khoan/:email/lich-su", yeuCauAdmin, async (req, res) => {
  try {
    res.json(
      await DonHang.find({ emailKhach: req.params.email.toLowerCase() })
        .select("maDon tenQuan thoiGianDat trangThai")
        .sort({ thoiGianDat: -1 })
        .lean(),
    );
  } catch {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      user.vipHetHan = congMotThang(
        user.vipHetHan > now ? user.vipHetHan : now,
      );
      await user.save();
      await ThongBao.create({
        emailKhach: user.email,
        noiDung: `Quản trị viên đã cấp VIP cho bạn đến ${user.vipHetHan.toLocaleDateString("vi-VN")}.`,
        loai: "he-thong",
      });
      res.json({
        message: `Đã cấp VIP đến ${user.vipHetHan.toLocaleDateString("vi-VN")}.`,
      });
    } catch {
      res.status(500).json({ message: "Không thể cấp VIP." });
    }
  },
);

router.post(
  "/admin/tai-khoan/:email/cong-diem",
  yeuCauAdmin,
  async (req, res) => {
    try {
      ...
      if (!Number.isSafeInteger(soDiem) || soDiem < 1 || soDiem > 1000000) {
        return res
          .status(400)
          .json({ message: "Số điểm phải là số nguyên từ 1 đến 1.000.000." });
      }
      const user = await NguoiDung.findOne({
        email: req.params.email.toLowerCase(),
      });
      ...
      res.json({
        ...
        laVip: duocCapVip || user.vipTrangThai === "VIP",
...
router.post(
  "/admin/tai-khoan/:email/thu-hoi-vip",
  yeuCauAdmin,
  async (req, res) => {
    try {
      ...
      if (!user)
        return res.status(404).json({ message: "Không tìm thấy khách hàng." });
      if (user.vipTrangThai !== "VIP")
        return res
          .status(409)
          .json({ message: "Tài khoản này hiện không có VIP để thu hồi." });
      user.vipTrangThai = "Chưa đăng ký";
      user.vipHetHan = null;
      await user.save();
      await ThongBao.create({
        emailKhach: email,
        noiDung: "Quyền VIP của bạn đã được Quản trị viên thu hồi.",
        loai: "he-thong",
      });
      res.json({ message: `Đã thu hồi VIP của ${user.hoTen}.` });
    } catch {
      res.status(500).json({ message: "Không thể thu hồi VIP." });
    }
  },
);
...
router.delete("/admin/tai-khoan/:email", yeuCauAdmin, async (req, res) => {
  try {
    ...
    await Promise.all([
      ...
      HoanTien.updateMany(
        { emailKhach: email },
        { $set: { emailKhach: "tai-khoan-da-xoa" } },
      ),
      DanhGia.deleteMany({ emailKhach: email }),
      BaiViet.deleteMany({ emailNguoiDang: email }),
      BaiViet.updateMany(
        { "binhLuan.emailNguoiBinhLuan": email },
        { $pull: { binhLuan: { emailNguoiBinhLuan: email } } },
      ),
      BaiViet.updateMany({ luotThich: email }, { $pull: { luotThich: email } }),
    ]);
    await NguoiDung.deleteOne({ _id: user._id });
    res.json({
      message: `Đã xóa tài khoản ${email} cùng thông báo, đánh giá và nội dung cộng đồng. Lịch sử đặt phòng/hoàn tiền vẫn được giữ cho quản trị và đã gỡ liên kết với email.`,
    });
  } catch {
