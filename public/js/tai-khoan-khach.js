async function taiThongTinTaiKhoan() {
  ...
  try {
    await Promise.all([taiLichSuDatPhong(), taiTrangThaiGoiVip(), taiYeuCauHoanTien()]);
  } catch (error) {
...
async function taiTrangThaiGoiVip() {
  const phanHoi = await fetch("/api/hoan-tien/vip-trang-thai", { credentials: "same-origin", cache: "no-store" });
  const vip = await phanHoi.json();
  if (!phanHoi.ok) throw new Error(vip.message || "Không tải được trạng thái gói VIP.");
...
async function taiYeuCauHoanTien() {
  ...
  const phanHoi = await fetch("/api/hoan-tien/yeu-cau-cua-toi", { credentials: "same-origin", cache: "no-store" });
  ...
  dau.innerHTML = "<tr><th>Mã đơn</th><th>Ngày gửi</th><th>Mức hoàn</th><th>Trạng thái</th></tr>";
  const than = document.createElement("tbody");
  yeuCaus.forEach((yc) => {
    const hang = document.createElement("tr");
    const ngay = yc.createdAt ? new Date(yc.createdAt).toLocaleDateString("vi-VN") : "—";
    const mucHoan = yc.phanTramHoan ? `${yc.phanTramHoan}%` : "Chờ nhân viên xét";
...
async function taiLichSuDatPhong() {
  ...
  const phanHoi = await fetch("/api/don-hang/lich-su-khach", { credentials: "same-origin", cache: "no-store" });
  ...
  bang.className = "bang-lich-su";
  const dau = document.createElement("thead");
  dau.innerHTML = "<tr><th>Mã đơn</th><th>Quán</th><th>Ngày đặt</th><th>Giờ đặt phòng</th><th>Trạng thái</th></tr>";
  const than = document.createElement("tbody");
  donHang.forEach((don) => {
    const hang = document.createElement("tr");
    const ngayDat = don.thoiGianDat
      ...
      : "—";
    [don.maDon, don.tenQuan, ngayDat, don.thoiGianCheckIn, don.trangThai].forEach((giaTri) => {
      const o = document.createElement("td");
      o.textContent = giaTri || "—";
      hang.append(o);
    });
...
document.querySelector("#nut-dang-xuat").addEventListener("click", async () => {
  ...
  nut.disabled = true;
  try {
    const phanHoi = await fetch("/api/dang-xuat", { method: "POST", credentials: "same-origin" });
...
taiThongTinTaiKhoan().catch((error) => {
  document.querySelector("#thong-bao").textContent = error.message || "Không tải được thông tin tài khoản.";
});
