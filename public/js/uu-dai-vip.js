...
async function taiDonDuDieuKien() {
  const phanHoi = await fetch("/api/hoan-tien/don-hang-cua-toi", {
...
async function khoiTaoUuDaiVip() {
  ...
  if (!phien.daDangNhap) {
    trangThai.textContent =
      "Đăng nhập tài khoản khách hàng để tham gia chương trình VIP.";
    document.querySelector("#yeu-cau-vip-dang-xuat").hidden = false;
    return;
  }
  const phanHoiVip = await fetch("/api/hoan-tien/vip-trang-thai", {
    credentials: "same-origin",
    cache: "no-store",
  });
  ...
  trangThai.textContent = `Tài khoản ${phien.email} là thành viên VIP · Quyền lợi hoàn tiền theo chính sách VIP hiện hành.`;
  formHoanTien.hidden = false;
  try {
...
formHoanTien.addEventListener("submit", async (event) => {
  ...
  if (!tep || tep.size > 5 * 1024 * 1024) {
    thongBaoHoanTien.textContent =
      "Chọn ảnh hóa đơn có dung lượng tối đa 5 MB.";
    return;
  }
  nutGui.disabled = true;
  ...
  try {
    ...
    if (!phanHoi.ok)
      throw new Error(ketQua.message || "Không gửi được yêu cầu.");
    ...
    nutGui.disabled = !chonDonHoanTien.options.some(
      (muc) => muc.value && !muc.disabled,
    );
  }
});

khoiTaoUuDaiVip().catch(() => {
  document.querySelector("#trang-thai-vip").textContent =
    "Không thể tải chương trình VIP lúc này.";
});
